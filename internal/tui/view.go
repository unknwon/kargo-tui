...
func (m Model) View() tea.View {
	...
	headerText := fmt.Sprintf("kargo-tui · %s · %s · project=%s · %d items · updated %s",
		title, m.contextName, m.project, count, ageString(m.lastUpdate))
	if m.loading {
	...
	help := lipgloss.NewStyle().Foreground(muted).Background(bg).Padding(0, 1).
		Render("d deploys · c controls · f freights · v details · l logs · D diff · o argo · s sort · y yank · n project · / filter · ? help · q quit")
